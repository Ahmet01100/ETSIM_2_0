...
if ($_SESSION['role'] == "Admin" || $_SESSION['role'] == "Manager" ) {
	...
	if (isset($_GET['id_plant']) && isset($_GET['colo']) && isset($_GET['delete'])) {
		if ($selectExistsP = $mysqli->prepare("SELECT * FROM is_type WHERE id_etsim_plant = ?")) {
			$selectExistsP->bind_param('s', $_GET['id_plant']);  
			$selectExistsP->execute();
			$resultselectExistsP = $selectExistsP->get_result();
			while($rowresultselectExistsP = $resultselectExistsP->fetch_assoc()) {
				$row_idP = $rowresultselectExistsP['id_etsim_plant'];
			}
			if ( $row_idP == $_GET['id_plant'] ) {
				$deletegameIS = $mysqli->prepare("DELETE FROM is_type WHERE id_etsim_plant = ?;");
				$deletegameIS->bind_param('s', $_GET['id_plant']);  
				$deletegameIS->execute();
				$deletegameP = $mysqli->prepare("DELETE FROM etsim_plant WHERE id_etsim_plant = ?;");
				$deletegameP->bind_param('s', $_GET['id_plant']);  
				$deletegameP->execute();
				$deletegameP->close();
				$deletegameIS->close();
