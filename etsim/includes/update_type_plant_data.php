...
if ($_SESSION['role'] == "Admin" || $_SESSION['role'] == "Manager" ) {
	...
	if(isset($_GET['id_type_plant']) && isset($_GET['colo']) && isset($_GET['delete'])){
		if ($selectExistsTP = $mysqli->prepare("SELECT * FROM is_type WHERE id_etsim_type_plant = :idTypePlant")) {
			$selectExistsTP->bindParam(':idTypePlant', $_GET['id_type_plant']);  
			$selectExistsTP->execute();
			//$resultselectExistsTP = $selectExistsTP->get_result();
			while($rowresultselectExistsTP = $selectExistsTP->fetch()) {
				$row_idTP = $rowresultselectExistsTP['id_etsim_type_plant'];
			}
			if ( $row_idTP == $_GET['id_type_plant'] ) {
				$deletegameIS = $mysqli->prepare("DELETE FROM is_type WHERE id_etsim_type_plant = :idTypePlant;");
				$deletegameIS->bindParam(':idTypePlant', $_GET['id_type_plant']);  
				$deletegameIS->execute();
				$deletegameP = $mysqli->prepare("DELETE FROM etsim_type_plant WHERE id_etsim_type_plant = :idTypePlant;");
				$deletegameP->bindParam(':idTypePlant', $_GET['id_type_plant']);  
				$deletegameP->execute();
				//$deletegameP->close();
				//$deletegameIS->close();
